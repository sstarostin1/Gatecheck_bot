...
| **Автономный пул v0.3.0** (4 GitHub-источника → 787 кандидатов → живая проба) | песочница (сеть с блокировкой TG) | ✅ 10 рабочих socks5 за ~4.5 мин (кэш сохранён); бот запустился и поллил через пул: `запущен как @Eve_gatechecker_bot (транспорт: socks5://…)` | **Автономный обход блокировки доказан**; успех ~5–20%, кэш протухает за минуты → ротация обязательна |
| zK `locationID` vs ESI stargates (OQ-2) | песочница | ✅ Amamake, 194 килла/сутки: 34 килла с locationID = itemID стargates (50004548–50004552, 50013705), 110 — станция 60004816, остальное — планеты/луны | **OQ-2 ЗАКРЫТ: D4 = locationID ∈ гейтов системы; геометрия не нужна (OQ-3 снят)** |
| ESI-граф Heimatar (fetch_static v1) | песочница | ✅ 83 системы / 238 гейта / 220 рёбер за ~7 с (конкурентность 8, gzip, UA) | Граф M1 собран; BFS: Amamake→Siseide 1 прыжок, Amamake→Rens 3 (через Osoggur, Abudban) |
...
- Открытые списки прокси (GitHub raw, доступен при блокировке TG): ~800 кандидатов за прогон,
  доля живых socks5 ≈ 5–20%, кэш протухает за минуты — отсюда требование ротации и повторной
  добычи (реализовано в v0.3.0). MTProto-секреты (ee/dd) в списках для Bot API бесполезны —
  фильтруются при парсинге. Идея автора про агрегаторы — OQ-13.
