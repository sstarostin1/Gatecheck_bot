...
### 5.1 Функциональный аудит vs VISION (2026-09-09, v0.6.0) — что осталось до «закончен»
...
автономный транспорт (сверх концепта); OQ 1,2,3,5,6,7,11,12 закрыты.

**Блок 1 — Персистентность (D2/§7/§8) — критично для G3 24/7:**
SQLite (WAL, миграции) для subscriptions/routes/kill_cache/alerts_log; сейчас ВСЁ состояние
in-memory: рестарт бота теряет /zone и /route подписки и историю киллов (базлайны обнуляются).

**Блок 2 — Релиз-инжиниринг (D8, G4) — критично для 24/7 и публикации:**
systemd-юнит Debian 13; GitHub Actions (ruff+pytest, автодеплой по SSH); публикация репо на
GitHub (сейчас только локально); README с пошаговым деплоем; UA/доки ссылаются на несуществующий
URL репо (OQ-10 хвост).

**Блок 3 — Гарантии публичного бота (D1/OQ-8, OQ-4) — перед открытием пользователям:**
fair-use лимиты (кол-во слежек, частота пушей на чат); /settings для порогов D5/cooldown
(сейчас пороги — константы кода); группировка ship+pod в один алерт (OQ-4).

**Финишные штрихи (маленькие):**
- /route-ответ при построении должен показывать ТЕКУЩИЕ счётчики киллов на гейтах каждой системы
  (§5, шаг 2 — сейчас счётчики только в /route status);
- в алертах маршрута добавить ссылку на zKillboard (есть в зональных, нет в маршрутных);
- /ping админа: статистика API + версия статики (§8);
- zK-poller мониторов: бэкофф на 429/403 (сейчас в monitors только log+skip; ретраи есть в fetch_static);
- автообновление статики раз в сутки / команда админа (§7).
