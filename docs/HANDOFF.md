...
### 5.2 Спека v0.10 — docs/MESSAGES.md (РЕАЛИЗОВАНА в v0.10.0, 2026-09-20)

Дизайн-спека всех выводимых сообщений (источник правды по текстам). Реализация по чек-листу §15:
1. **HTML parse mode** (`DefaultBotProperties(parse_mode=ParseMode.HTML)`); `render.py` —
   разметка `<b>/<i>/<pre>/<blockquote expandable>`, экранирование, футеры с EVE Time.
2. **10-минутные триггеры удалены** (§0.7): остались только часовые счётчики; /settings —
   hour/isk/cooldown (+ attackers on/off), burst/isk_burst вычищены.
3. **Дельта «(+N за последние 10 минут)»** — скользящее окно по таймстампам киллов.
4. **Зона §1–§4**: заголовки «Сейчас гайки кемпят в системе/системах:», quote-блоки систем
   (первая строка «{система} N киллов и {ISK} на гейтах», пер-гейт «на {назначение}: N за час
   (+M за последние 10 минут), droppable …» + «атака: соло/коллективный · корабли»), сортировка
   блоков по убыванию киллов, «✅ Чисто: X/Y систем», /zone_status без подписки = живой опрос.
5. **Маршрут §6–§8**: старт /route — ОДНО сообщение (вердикт 4 уровней → `<pre>`-список систем
   с sec-статусами для EVE Notepad → первый опрос → футер), затем алерт-цикл; ПУШИ только по
   🚩-маршрутным гейтам (гейты в ±1 систему по маршруту), КД нет; /route_status с вердиктом.
6. **Атака/бомбы/бабблы (§7)**: тип кемпа по zkb.solo, корабли атакующих (ESI names), 💣 по
   weapon_type_id ∈ SDE-группа 55 (ESI /universe/groups/55), 🫧 признаки — дикторы/HIC или
   ISL/WDFG; toggle /settings_attackers.
7. **Кнопки §13**: emoji-лейблы, F.text-маппинг; «🔄 Гейты: обновить» — тик ТОЛЬКО зон + отчёт;
   inline «🔄 Обновить» (refresh:zone) — только на зональных репортах.
8. **Команды с подчёркиванием** (+ старые формы с пробелом), /ping — только админам (молчание),
   /start и /help по §9–§10; SQLite-миграция 002 (features_json) для персистентности признаков.
