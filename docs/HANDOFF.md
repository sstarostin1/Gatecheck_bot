...
6. Блоки систем — expandable quote: первая строка «В {система} N килов и {ISK} на гейтах»,
   ниже пер-гейт «на {система-назначения}: N за час ({±N от прошлого репорта}), droppable {ISK}»
...
7. Дельта «±N от прошлого репорта» — новая SQLite-таблица report_counts.
Чек-лист реализации — §14 спеки (HTML parse mode, quote, дельта, подчёркивания, /ping).
