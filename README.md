...
**Текущий статус (v0.6.0):** бот работает при блокировках (автономный прокси-пул), маршруты по
всему Новому Эдену + слежение за гейт-кампами на маршруте (`/route Amamake Jita`, 15 прыжков) и
**мониторинг зоны фарма**: `/zone on` — пресет «Hed + соседи» (13 систем), алерты всплесков
(≥3 килла/10 мин, ≥8/час, droppable ISK), `/zone status` — счётчики по системам. План —
...
Затем в Telegram: `/start` → приветствие; `/route Amamake Jita` → маршрут (15 прыжков) **и слежение
за гейт-кампами на нём** (TTL 1 ч, алерты о новых киллах на гейтах, `/route status` — статистика,
`/route stop` — выключить); `/zone on` — мониторинг зоны фарма «Hed + соседи» (алерты всплесков
на гейтах, `/zone status` — счётчики); любой текст → эхо.
