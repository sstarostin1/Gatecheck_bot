...
**Текущий статус (v0.5.0):** бот работает при блокировках (автономный прокси-пул), маршруты по
всему Новому Эдену + слежение за гейт-кампами на маршруте: `/route Amamake Jita` (15 прыжков),
`/route status` — статистика киллов на гейтах с прогрессией, `/route stop`. Мониторинг зон (M2) —
следующий этап, план в [docs/VISION.md](docs/VISION.md).
...
`/route stop` — выключить); любой текст → эхо.
